...
function App() {
  ...
  useEffect(() => {
    ...
    if (busy && longOps.includes(busy)) {
      ...
      progressTimerRef.current = setInterval(() => {
        ...
        if (pct < 35)
          setProgressLabel(
            busy === 'extract' ? '大模型正在分析业务规则...' : '大模型正在生成测试用例草稿...',
          );
        else if (pct < 65)
          setProgressLabel(
            busy === 'extract' ? '正在处理 AI 返回内容...' : '正在校验和保存生成结果...',
          );
  ...
  const [latestSupplementResult, setLatestSupplementResult] = useState<DiffSupplementCaseVO | null>(
    null,
  );
  ...
  const generationId =
    ...
    requirementExtract?.generationId ||
  ...
  const refreshGeneration = async () => {
    ...
    const data = await runAction('generation', '查询生成记录', () =>
  ...
  const refreshCases = async () => {
    ...
    try {
      const data = await runAction('cases', '查询正式用例', () => api.listCases(queryInfo));
  ...
  const createDiffTask = async () => {
    ...
    const data = await runAction('diff-create', '创建 Diff 分析任务', () =>
      api.createDiffTask({
        ...
        requirementExtractId:
          values.requirementExtractId || requirementExtract?.requirementExtractId,
  ...
  const refreshDiffTasks = async () => {
    ...
    const selectedRequirementExtractId =
      diffForm.getFieldValue('requirementExtractId') || requirementExtract?.requirementExtractId;
    const data = await runAction('diff-list', '查询 Diff 分析任务', () =>
      api.listDiffTasks({
        documentId: selectedDocumentId,
        requirementExtractId: selectedRequirementExtractId,
      }),
    );
  ...
  const refreshRepositoryBranches = async () => {
    ...
    const data = await runAction('diff-branches', '查询仓库分支', () =>
      api.listRepositoryBranches(repoUrl),
    );
    if (data) {
      setDiffBranches(data);
      if (!diffForm.getFieldValue('targetBranch')) {
        diffForm.setFieldValue(
          'targetBranch',
          data.includes('master') ? 'master' : data.includes('main') ? 'main' : data[0],
        );
      }
    }
  };
  ...
  const loadDiffReport = async (taskId: number) => {
    ...
    const data = await runAction('diff-report', '查询 Diff 分析报告', () =>
      api.getDiffReport(taskId),
    );
  ...
  const generateSupplementCases = async (risk: DiffRiskItemVO) => {
    const data = await runAction('diff-supplement', '生成补充用例', () =>
      api.generateDiffSupplementCases(risk.riskId),
    );
    if (data) {
  ...
  const saveDraft = async () => {
    ...
    const caseType =
      toEnglishCaseType(values.caseType) ||
      toEnglishCaseType(getDefaultCaseTypeForRiskLevel(riskLevel)) ||
      'NORMAL';
  ...
  const rejectDraft = async (draft: TestCaseDraftVO) => {
    const data = await runAction('reject-draft', '驳回草稿', () =>
  ...
  const chunkColumns: ColumnsType<DocumentChunkVO> = [
    { title: 'Chunk ID', dataIndex: 'chunkId', width: 180, fixed: 'left' },
    {
      ...
      width: 80,
      sorter: (a, b) => a.chunkIndex - b.chunkIndex,
    },
    {
      title: '所属章节',
      dataIndex: 'sectionTitle',
      width: 160,
      render: (value) => value || '默认章节',
    },
  ...
  const draftColumns: ColumnsType<TestCaseDraftVO> = [
    ...
    },
    {
      ...
      dataIndex: 'caseType',
      width: 130,
      render: (value) => <Tag color="purple">{formatCaseType(value)}</Tag>,
    },
    {
      title: '风险等级',
      dataIndex: 'riskLevel',
    ...
    {
      title: '关联需求',
      dataIndex: 'requirementRefsJson',
      width: 180,
      render: (value) => renderJsonTags(value),
    },
    {
      title: '操作',
      width: 230,
  ...
  const caseColumns: ColumnsType<TestCaseVO> = [
    ...
    },
    {
      ...
      dataIndex: 'status',
      width: 100,
      render: (value) => <Tag color="green">{value || 'ACTIVE'}</Tag>,
    },
    {
      title: '关联需求',
      dataIndex: 'requirementRefsJson',
        ...
        <Content className="app-content">
          <div className="content-grid">
            <main className="workbench-main">
              ...
              <Spin
                spinning={busy !== null && busy !== 'extract' && busy !== 'generate'}
                tip={loadingText(busy)}
              >
      ...
      <footer className="app-footer">
        <div className="footer-content">
          <div className="footer-section">
            ...
            <a href="https://autotest.wiac.xyz/" target="_blank" rel="noopener noreferrer">
              https://autotest.wiac.xyz/
            </a>
          </div>
          ...
          <div className="footer-section">
            ...
            <a
              href="https://github.com/acai1998/AI-TestOps"
              target="_blank"
              rel="noopener noreferrer"
            >
              https://github.com/acai1998/AI-TestOps
            </a>
          </div>
        </div>
      </footer>
            ...
            onValuesChange={(changedValues) => {
              if (changedValues.riskLevel) {
                draftForm.setFieldValue(
                  'caseType',
                  getDefaultCaseTypeForRiskLevel(changedValues.riskLevel),
                );
              }
            }}
          >
            ...
            <Form.Item
              label="前置条件 JSON"
              name="preconditionsJson"
              rules={[jsonArrayRule('前置条件', { allowEmpty: true })]}
            >
              <TextArea rows={4} />
            </Form.Item>
            <Form.Item
              label="测试步骤 JSON"
              name="stepsJson"
              rules={[jsonArrayRule('测试步骤', { allowEmpty: false })]}
            >
              <TextArea rows={8} />
            </Form.Item>
            <Form.Item
              ...
              rules={[jsonArrayRule('预期结果', { allowEmpty: false })]}
            >
              <TextArea rows={4} />
            </Form.Item>
            <Form.Item
              label="关联需求 JSON"
              name="requirementRefsJson"
              rules={[jsonArrayRule('关联需求', { allowEmpty: true })]}
            >
              <TextArea rows={3} />
            </Form.Item>
            <Form.Item
              label="风险标签 JSON"
              name="riskTagsJson"
              rules={[jsonArrayRule('风险标签', { allowEmpty: true })]}
            >
              <TextArea rows={3} />
            </Form.Item>
          </Form>
              ...
              children: (
                <div className="stack">
                  ...
                  <Space wrap>
                    <Button
                      icon={<CodeOutlined />}
                      onClick={() => setTextContent(exampleRequirement)}
                    >
                      填充示例
                    </Button>
                ...
                <div className="stack">
                  ...
                  <Dragger {...uploadProps}>
                    ...
                    <p className="ant-upload-hint">
                      支持 md / txt / doc / docx / pdf / ppt / pptx / xls / xlsx，单文件最大 50MB
                    </p>
                  </Dragger>
                  {selectedFile ? (
                    <FileCard file={selectedFile} />
                  ) : (
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无需求文件" />
                  )}
                  <Button
                    type="primary"
    ...
    return (
      <div className="stack">
                ...
                <Col xs={24} lg={12}>
                  <Card size="small" title="元数据" className="soft-card">
                    <pre className="json-box">
                      {formatJson(documentInfo.metadataJson || parseSummary?.metadataJson)}
                    </pre>
                  </Card>
                </Col>
                <Col xs={24} lg={12}>
        ...
        <Card
          className="panel-card"
          title="Chunk 分块结果"
          extra={<Tag>{chunks.length} 个 chunks</Tag>}
        >
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} md={8}>
              ...
              <InputNumber
                ...
                min={1}
                max={50}
                onChange={(val) => setCaseCount(val ?? 20)}
              />
            </Col>
          </Row>
          <Divider />
        ...
        >
          <Row gutter={[16, 16]}>
            <Col xs={24} md={8}>
              <Statistic
                title="结构化需求"
                value={countJsonItems(requirementExtract?.requirementsJson)}
                suffix="条"
              />
            </Col>
            <Col xs={24} md={8}>
          ...
          extra={
            <Space wrap>
              ...
              <Button
                ...
                onClick={() => window.open(api.exportJsonUrl(documentId), '_blank')}
              >
                导出 JSON
              </Button>
              <Button
                type="primary"
                icon={<DownloadOutlined />}
                onClick={() => window.open(api.exportExcelUrl(documentId), '_blank')}
              >
                导出 Excel
              </Button>
            </Space>
    ...
    return (
      <Card title="当前结果摘要" className="side-card">
        <Space direction="vertical" className="full-width" size="middle">
          ...
          <SummaryItem
            label="文档状态"
            value={documentInfo?.parseStatus || '未开始'}
            status={documentInfo?.parseStatus}
          />
          <SummaryItem
            label="文本长度"
  ...
  function renderDiffStep() {
    ...
    const taskColumns: ColumnsType<DiffAnalysisTaskVO> = [
      { title: '任务编码', dataIndex: 'taskCode', width: 180 },
      {
        ...
        dataIndex: 'repoName',
        width: 160,
        render: (value, record) => value || record.repoUrl,
      },
      ...
      {
        title: '准入',
        dataIndex: 'mergeGateStatus',
        width: 130,
        render: (value) => <GateTag value={value} />,
      },
          ...
          >
            <Row gutter={[16, 0]}>
                    ...
                    dropdownRender={(menu) => (
                      <>
                        ...
                        <Button
                          type="link"
                          icon={<ReloadOutlined />}
                          loading={busy === 'diff-sources'}
                          onClick={refreshDiffSources}
                        >
                          {'刷新最近来源'}
                        </Button>
                      </>
              ...
              <Col xs={24} md={12}>
                <Form.Item name="requirementExtractId" label="需求解析结果">
                  <Input
                    placeholder={
                      requirementExtract?.requirementExtractId ||
                      '留空时自动匹配文档最近一次解析结果'
                    }
                  />
                </Form.Item>
              ...
              <Col xs={24}>
                <Form.Item
                  name="repoUrl"
                  label="仓库地址"
                  rules={[{ required: true, message: '请输入 Git 仓库地址或本地仓库路径' }]}
                >
                  <Input placeholder="https://github.com/acai1998/AI-TestOps.git" />
                </Form.Item>
                <Button
                  icon={<ReloadOutlined />}
                  loading={busy === 'diff-branches'}
                  onClick={refreshRepositoryBranches}
                >
                  {'获取仓库分支'}
                </Button>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item
                  name="sourceBranch"
                  label="变更分支（待分析）"
                  rules={[{ required: true, message: '请输入变更分支' }]}
                >
                  <AutoComplete
                    options={branchOptions}
                    placeholder="feature/diff-analysis-task"
                    filterOption
                  />
                </Form.Item>
              </Col>
            ...
            <Space wrap>
              <Button
                type="primary"
                icon={<PlayCircleOutlined />}
                loading={busy === 'diff-create'}
                onClick={createDiffTask}
              >
                {'开始分析'}
              </Button>
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={12} md={6}>
                <Statistic
                  title="准入结论"
                  value={diffReport.report?.gateStatus || diffReport.task.mergeGateStatus || '-'}
                />
              </Col>
              <Col xs={12} md={6}>
                <Statistic
                  title="变更文件"
                  value={diffReport.report?.changedFileCount || diffReport.changedFiles.length}
                  suffix="个"
                />
              </Col>
              <Col xs={12} md={6}>
                <Statistic
                  title="高风险"
                  value={diffReport.report?.highRiskCount || 0}
                  suffix="项"
                />
              </Col>
              <Col xs={12} md={6}>
                <Statistic
                  title="未覆盖"
                  value={diffReport.report?.notCoveredRiskCount || 0}
                  suffix="项"
                />
              </Col>
            </Row>
            <Divider />
            <Alert
              type={diffReport.report?.gateStatus === 'BLOCK' ? 'error' : 'info'}
              showIcon
              message={diffReport.report?.gateReason || '暂无准入原因'}
            />
          </Card>
        )}
      </div>
  ...
  function renderDiffDetailStep() {
    ...
    const changedFileColumns: ColumnsType<DiffChangedFileVO> = [
      ...
      {
        title: '初判',
        dataIndex: 'initialRiskLevel',
        width: 110,
        render: (value) => <RiskLevelTag value={value} />,
      },
      {
        title: '操作',
        width: 90,
    ...
    const riskColumns: ColumnsType<DiffRiskItemVO> = [
      ...
      },
      {
        ...
        width: 140,
        render: (value) => <CoverageTag value={value} />,
      },
      {
        title: '处理',
      ...
      },
      {
        ...
        width: 180,
        render: (_, record) =>
          record.matchedCases?.length ? `${record.matchedCases.length} 条` : '-',
      },
      {
        title: '操作',
        width: 300,
...
function validateFile(file: File) {
  ...
  if (!allowedExtensions.includes(extension))
    return `文件格式不支持，请上传 ${allowedExtensions.join(' / ')}。`;
  return '';
}
...
function StatusTag({ value }: { value?: string }) {
  ...
  return (
    <Tag color={value ? colorMap[value] || 'default' : 'default'}>
      {value ? labelMap[value] || value : '未开始'}
    </Tag>
  );
}
...
function RiskTag({ value }: { value?: string }) {
  const color =
    value === 'P0' ? 'red' : value === 'P1' ? 'orange' : value === 'P2' ? 'green' : 'default';
  const label =
    value === 'P0' ? 'P0' : value === 'P1' ? 'P1' : value === 'P2' ? 'P2' : value || '-';
...
function draftToForm(draft: TestCaseDraftVO) {
  ...
  const normalizedExpectedResultsJson = normalizeExpectedResultsJsonForEditor(
    draft.stepsJson,
    draft.expectedResultsJson,
  );
  return {
    title: draft.title,
    priority: draft.priority || 'P1',
    caseType: draft.caseType
      ? formatCaseType(draft.caseType)
      : getDefaultCaseTypeForRiskLevel(riskLevel),
    riskLevel,
    ...
    const expectedResults = parsed
      .map((item) =>
        item && typeof item === 'object' && !Array.isArray(item)
          ? (item as Record<string, unknown>).expected_result
          : undefined,
      )
